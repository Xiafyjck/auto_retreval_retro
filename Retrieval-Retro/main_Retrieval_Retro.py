...
def main():
    ...
    args.eval = 5
    args.retrieval = 'RetroPLEX'
    args.embedder = 'Retrieval_Retro'

    train_dataset = torch.load(f'./dataset/{args.split}/train_K_{args.K}.pt',map_location=device, weights_only=False)
    valid_dataset = torch.load(f'./dataset/{args.split}/valid_K_{args.K}.pt',map_location=device, weights_only=False)
    test_dataset = torch.load(f'./dataset/{args.split}/test_K_{args.K}.pt',map_location=device, weights_only=False)
